...
func _tick(_delta: float) -> Status:
	var facing: Vector3 = agent.get_facing()
	var speed: float = blackboard.get_var(speed_var, 40.0)
	var desired_velocity: Vector3 =  facing * speed
	agent.move(desired_velocity)
	agent.face_dir(desired_velocity)
	var str_vel = var_to_str(desired_velocity)
	
	agent.set_text("MOVE FORWARD " + str_vel)
	#agent.update_facing()
